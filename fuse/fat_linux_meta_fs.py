...
Backing store layout:
  ...
  <backing>/<meta-file>   — JSON metadata (default: .linux-meta.json)
...
class FatLinuxMetaFS(fuse.Operations):
    """Passthrough with metadata overlay persisted to one JSON file."""
    ...
    def __init__(self, backing_root: str, meta_file: str) -> None:
        ...
        self.meta_path = (
            ...
            else os.path.join(self.backing_root, meta_file)
        )
        self._lock = threading.Lock()
        self._meta: dict[str, Any] = {"version": META_VERSION, "entries": {}}
        self._load()
        if self._meta.get("version") != META_VERSION:
            self._meta["version"] = META_VERSION
    ...
    def _save(self) -> None:
        ...
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump(self._meta, f, indent=0, sort_keys=True)
            f.write("\n")
    ...
    def _get_entry(self, path: str) -> dict[str, Any] | None:
        key = _norm_rel(path)
        ent = self._meta.get("entries", {}).get(key)
        return ent if isinstance(ent, dict) else None

    def _set_entry(self, path: str, updates: dict[str, Any]) -> None:
        key = _norm_rel(path)
        with self._lock:
            entries = self._meta.setdefault("entries", {})
            cur = entries.get(key, {})
    ...
    def _del_entry(self, path: str) -> None:
        key = _norm_rel(path)
        with self._lock:
            self._meta.setdefault("entries", {}).pop(key, None)
        self._save()

    def _rename_entries_prefix(self, old_prefix: str, new_prefix: str) -> None:
        old_k = _norm_rel(old_prefix)
        new_k = _norm_rel(new_prefix)
        with self._lock:
            entries = self._meta.setdefault("entries", {})
            to_move: list[tuple[str, dict[str, Any]]] = []
            keys = list(entries.keys())
            for k in keys:
                if k == old_k or (old_k != "" and (k == old_k or k.startswith(old_k + os.sep))):
                    ent = entries.pop(k)
                    if k == old_k:
                        nk = new_k
                    else:
                        suffix = k[len(old_k) :].lstrip("/")
                        nk = os.path.join(new_k, suffix) if new_k else suffix
                    nk = os.path.normpath(nk)
                    to_move.append((nk, ent))
            for nk, ent in to_move:
                entries[nk] = ent
        self._save()
    ...
    def _fuse_hide_chunk_path(self, path: str) -> str | None:
        """Return path if the basename is a chunk sidecar (hidden from FUSE)."""
        if path in ("/", ""):
            return None
        if _is_chunk_part_filename(os.path.basename(path.rstrip("/"))):
            return path
        return None

    def getattr(self, path: str, fh: Any = None) -> dict[str, Any]:
        if self._fuse_hide_chunk_path(path):
    ...
    def readdir(self, path: str, fh: Any) -> list[str]:
        ...
        names = [n for n in os.listdir(full) if not _is_chunk_part_filename(n)]
        return [".", "..", *names]
    ...
    def create(self, path: str, mode: int, fi: Any = None) -> int:
        ...
        fd = os.open(full, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, mode & 0o777)
        u = os.geteuid()
        g = os.getegid()
        self._set_entry(
            path,
    ...
    def write(self, path: str, buf: bytes, offset: int, fh: int) -> int:
        ...
        if (
            ...
            return self._write_virtual(path, buf, offset)
        os.lseek(fh, offset, os.SEEK_SET)
    ...
    def rename(self, old: str, new: str) -> int:
        if self._fuse_hide_chunk_path(old) or self._fuse_hide_chunk_path(new):
            raise fuse.FuseOSError(errno.EPERM)
        old_rel = _norm_rel(old)
        new_rel = _norm_rel(new)
        self._rename_chunk_files(old_rel, new_rel)
        self._rename_entries_prefix(old, new)
        return 0
    ...
    def chmod(self, path: str, mode: int) -> int:
        ...
        if cur_mode is None and os.path.exists(full):
    ...
    def symlink(self, target: str, source: str) -> int:
        ...
        if self._fuse_hide_chunk_path(target):
            raise fuse.FuseOSError(errno.EPERM)
    ...
    def link(self, target: str, source: str) -> int:
        ...
        if self._fuse_hide_chunk_path(target) or self._fuse_hide_chunk_path(source):
            raise fuse.FuseOSError(errno.EPERM)
    ...
    def mknod(self, path: str, mode: int, dev: int) -> int:
        ...
        self._set_entry(
            path,
            {"mode": mode, "uid": u, "gid": g, "rdev": dev},
        )
        return 0
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="FUSE passthrough with Linux metadata in a sidecar JSON file (for FAT backends)."
    )
