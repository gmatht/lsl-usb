...
fn inner_message(parent: HWND, params: &MessageParams) -> MessageChoice {
    use winapi::um::winuser::{MB_ABORTRETRYIGNORE, MB_CANCELTRYCONTINUE, MB_OK, MB_OKCANCEL, MB_RETRYCANCEL, MB_YESNO,
       ...
       // Win95 patch: ANSI message box (MessageBoxW is a stub on Win95)
       let text = to_ansi(params.content);
       let title = to_ansi(params.title);
       ...
       let answer = unsafe{ MessageBoxA(parent, text.as_ptr() as *const i8, title.as_ptr() as *const i8, buttons | icons) };
       match answer {
