...
unsafe extern "system" fn process_events(hwnd: HWND, msg: UINT, w: WPARAM, l: LPARAM, id: UINT_PTR, data: DWORD_PTR) -> LRESULT {
    ...
    match msg {
        ...
        WM_COMMAND => {
            ...
            let count = GetClassNameA(child_handle, class_name_raw.as_mut_ptr() as *mut i8, 100) as usize;
            let class_name = from_ansi(&class_name_raw[..count]);
...
fn tree_data(m: u32, notif_raw: *const NMHDR) -> EventData {
    ...
    match m {
        TVN_DELETEITEMW => {
            let data: NMTREEVIEWW = unsafe { std::ptr::read_unaligned(notif_raw as *const NMTREEVIEWW) }; // (Patch-Win95) unaligned
        ...
        TVN_ITEMCHANGEDW => {
            let data: NMTVITEMCHANGE = unsafe { std::ptr::read_unaligned(notif_raw as *const NMTVITEMCHANGE) }; // (Patch-Win95) unaligned
        ...
        TVN_ENDLABELEDITW => {
            let data: NMTVDISPINFOW = unsafe { std::ptr::read_unaligned(notif_raw as *const NMTVDISPINFOW) }; // (Patch-Win95) unaligned
...
fn list_view_data(m: u32, notif_raw: *const NMHDR) -> EventData {
    ...
    match m {
        LVN_DELETEITEM | LVN_INSERTITEM | LVN_COLUMNCLICK => {
            let data: NMLISTVIEW = unsafe { std::ptr::read_unaligned(notif_raw as *const NMLISTVIEW) }; // (Patch-Win95) unaligned
        ...
        LVN_ITEMACTIVATE | NM_CLICK | NM_DBLCLK | NM_RCLICK => {
            let data: NMITEMACTIVATE = unsafe { std::ptr::read_unaligned(notif_raw as *const NMITEMACTIVATE) }; // (Patch-Win95) unaligned
            EventData::OnListViewItemIndex { 
                row_index: data.iItem as _,
                column_index: data.iSubItem as _
            }
...
unsafe fn static_commands(handle: HWND, m: u16) -> Event {
    ...
    let has_image = SendMessageA(handle, STM_GETIMAGE, IMAGE_BITMAP as usize, 0) != 0;
    let has_icon = SendMessageA(handle, STM_GETIMAGE, IMAGE_ICON as usize, 0) != 0;
    let has_cursor = SendMessageA(handle, STM_GETIMAGE, IMAGE_CURSOR as usize, 0) != 0;
...
unsafe fn handle_default_notify_callback<'a>(notif_raw: *const NMHDR, callback: &Callback){
    ...
    let mut class_name_raw: [u8; 100] = mem::zeroed();
    let count = GetClassNameA(notif.hwndFrom, class_name_raw.as_mut_ptr() as *mut i8, 100) as usize;
    let class_name = from_ansi(&class_name_raw[..count]);
