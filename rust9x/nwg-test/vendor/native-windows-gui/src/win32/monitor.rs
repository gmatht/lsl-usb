...
use winapi::um::winuser::{GetSystemMetrics, MONITORINFO,
...
impl Monitor {
    ...
    fn monitor_info_from_window(_handle: HWND) -> MONITORINFO {
        // Win95 has no multi-monitor support: MonitorFromWindow/GetMonitorInfoW
        // require Windows 98 or later (statically importing them makes the binary
        // unloadable on Windows 95). Fall back to the primary display geometry.
        unsafe {
            ...
            info.rcMonitor.right = cx;
            info.rcMonitor.bottom = cy;
            info.rcWork.left = 0;
            info.rcWork.top = 0;
            info.rcWork.right = cx;
            info.rcWork.bottom = cy;
