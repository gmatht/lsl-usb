...
use winapi::um::winuser::WNDCLASSEXA;
...
#[cfg(feature = "rich-textbox")]
pub fn get_class_info(hwnd: HWND) -> Result<WNDCLASSEXA, ()> {
    use winapi::um::winuser::{GetClassInfoExA, GetClassNameA};
    use winapi::um::libloaderapi::GetModuleHandleA;

    unsafe {
        let mut info: WNDCLASSEXA = mem::zeroed();

        let hinst = GetModuleHandleA(ptr::null_mut());

        // Win95 patch: ANSI class name
        let mut class_name_raw: [u8; 100] = [0; 100];
        let count = GetClassNameA(hwnd, class_name_raw.as_mut_ptr() as *mut i8, 100) as usize;
        if count == 0 {
            return Err(());
        }

        let result = GetClassInfoExA(hinst, class_name_raw.as_ptr() as *const i8, &mut info);
        match result == 0 {
            true => Err(()),
            false => Ok(info)
        }
...
pub unsafe fn set_window_font(handle: HWND, font_handle: Option<HFONT>, redraw: bool) {
    ...
    SendMessageA(handle, WM_SETFONT, mem::transmute(font_handle), redraw as LPARAM);
}
...
pub fn send_message(hwnd: HWND, msg: UINT, w: WPARAM, l: LPARAM) -> LRESULT {
    unsafe { ::winapi::um::winuser::SendMessageA(hwnd, msg, w, l) }
}

pub fn post_message(hwnd: HWND, msg: UINT, w: WPARAM, l: LPARAM) {
    unsafe { ::winapi::um::winuser::PostMessageA(hwnd, msg, w, l) };
}
...
pub unsafe fn set_window_size(handle: HWND, w: u32, h: u32, fix: bool) {
    use winapi::um::winuser::{SetWindowPos, AdjustWindowRectEx, GetWindowLongA};
    ...
    if fix {
        // Win95 patch: ANSI style APIs
        let flags = GetWindowLongA(handle, GWL_STYLE) as u32;
        let ex_flags = GetWindowLongA(handle, GWL_EXSTYLE) as u32;
...
pub unsafe fn get_window_class_name(handle: HWND) -> String {
    // Win95 patch: ANSI class name
    let mut class_name_raw: Vec<u8> = Vec::with_capacity(100); 
    class_name_raw.resize(100, 0);

    let count = ::winapi::um::winuser::GetClassNameA(handle, class_name_raw.as_mut_ptr() as *mut i8, 100) as usize;
    
    from_ansi(&class_name_raw[..count])
}
...
pub fn get_window_long(handle: HWND, index: c_int) -> LONG_PTR {
    unsafe{ ::winapi::um::winuser::GetWindowLongPtrA(handle, index) }
}

#[inline(always)]
#[cfg(target_pointer_width = "32")]
pub fn get_window_long(handle: HWND, index: c_int) -> LONG {
    // Win95 patch: GetWindowLongW is a stub on Win95
    unsafe { ::winapi::um::winuser::GetWindowLongA(handle, index) }
}

#[inline(always)]
#[cfg(target_pointer_width = "64")]
pub fn set_window_long(handle: HWND, index: c_int, v: usize) {
    unsafe{ ::winapi::um::winuser::SetWindowLongPtrA(handle, index, v as LONG_PTR); }
}

#[inline(always)]
#[cfg(target_pointer_width = "32")]
pub fn set_window_long(handle: HWND, index: c_int, v: usize) {
    // Win95 patch: SetWindowLongW is a stub on Win95
    unsafe { ::winapi::um::winuser::SetWindowLongA(handle, index, v as LONG); }
}
