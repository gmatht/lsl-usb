...
impl<D: Display+Default> ComboBox<D> {
    ...
    fn hook_non_client_size(&self, bg: Option<[u8; 3]>, v_align: VTextAlign) {
        ...
        use winapi::um::winuser::{WM_NCCALCSIZE, WM_NCPAINT, WM_SIZE, NCCALCSIZE_PARAMS, COLOR_WINDOW};
        ...
        let handler0 = bind_raw_event_handler_inner(&self.handle, 0, move |hwnd, msg, w, l| {
            match msg {
                WM_NCCALCSIZE  => {
                    ...
                    // DrawTextW/DT_CALCRECT (see label.rs for details).
                    ...
                    let client_height = tm.tmHeight + tm.tmExternalLeading - 5; // 5 is the combobox padding
                    SelectObject(dc, old);
                    ReleaseDC(hwnd, dc);
