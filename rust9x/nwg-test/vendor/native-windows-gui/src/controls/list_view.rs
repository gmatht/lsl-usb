...
impl ListView {
    ...
    pub fn column_sort_arrow(&self, column_index: usize) -> Option<ListViewColumnSortArrow> {
        ...
        // (Patch-Win95) ANSI header messages: Win95 comctl32 has no W support
        let mut header: HDITEMA = unsafe { mem::zeroed() };
        header.mask = HDI_FORMAT;

        let l = &mut header as *mut HDITEMA as _;
        wh::send_message(headers as *mut _, HDM_GETITEMA, column_index, l);
    ...
    pub fn set_column_sort_arrow(&self, column_index: usize, sort: Option<ListViewColumnSortArrow>) {
        ...
        if headers != 0 {
            ...
            let l = &mut header as *mut HDITEMA as _;
            wh::send_message(headers as *mut _, HDM_SETITEMA, column_index, l);
        }
    }
