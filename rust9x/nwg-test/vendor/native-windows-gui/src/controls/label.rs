...
impl Label {
    ...
    fn hook_non_client_size(&mut self, bg: Option<[u8; 3]>, v_align: VTextAlign) {
        ...
        use winapi::um::winuser::{WM_CTLCOLORSTATIC, WM_NCCALCSIZE, WM_NCPAINT, WM_SIZE, NCCALCSIZE_PARAMS, COLOR_WINDOW};
        use winapi::um::winuser::{SWP_NOOWNERZORDER, SWP_NOSIZE, SWP_NOMOVE, SWP_FRAMECHANGED};
        use winapi::um::winuser::{GetDC, ReleaseDC, GetClientRect, GetWindowRect, FillRect, ScreenToClient, SetWindowPos};
        use winapi::um::wingdi::{SelectObject, CreateSolidBrush, RGB, GetTextMetricsA, TEXTMETRICA};
        use std::{mem, ptr};
        ...
        let handler1 = bind_raw_event_handler_inner(&self.handle, 0, move |hwnd, msg, w, l| {
            match msg {
                WM_NCCALCSIZE  => {
                    ...
                    // (Patch-Win95) Measure the text line height with
                    // GetTextMetricsA: DrawTextW is a no-op stub on Win95 (not
                    // in the working GDI W subset), so DT_CALCRECT left the
                    // rect zeroed and the client area collapsed to 0 — labels
                    // vanish after any SetWindowPos (relayout).
                    let font_handle = wh::get_window_font(hwnd);
                    let mut tm: TEXTMETRICA = mem::zeroed();
                    let dc = GetDC(hwnd);
                    ...
                    // panic in debug builds. Copy in/out with unaligned access.
                    let mut info: NCCALCSIZE_PARAMS = ptr::read_unaligned(info_ptr);
                    match v_align {
                        VTextAlign::Top => {
                    ...
                    ptr::write_unaligned(info_ptr, info);
                },
                WM_NCPAINT  => {
                    let mut window: RECT = mem::zeroed();
                    let mut client: RECT = mem::zeroed();
