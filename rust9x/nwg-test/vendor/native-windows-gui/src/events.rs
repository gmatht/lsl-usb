...
impl MinMaxInfo {
    ...
    /// (Patch-Win95) Win95 USER passes 2-byte-aligned 16-bit-heap pointers in
    /// WM_GETMINMAXINFO lParam; an aligned reference would panic in debug
    /// builds. Copy in/out with unaligned access.
    #[inline]
    fn with_info<R>(&self, f: impl FnOnce(&mut MINMAXINFO) -> R) -> R {
        let mut info: MINMAXINFO = unsafe { std::ptr::read_unaligned(self.inner) };
        let r = f(&mut info);
        unsafe { std::ptr::write_unaligned(self.inner, info) };
        r
    }
